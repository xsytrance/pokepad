/*
 * Poképad — full Gen-III battle engine in Kotlin (the on-device implementation).
 * A faithful port of sim/engine.py + sim/abilities.py. The deterministic math is
 * cross-gated bit-for-bit to Python (Engine.kt / CrossGate.kt); the battle-loop
 * control flow is ported here and self-tested (RNG differs from Python, so
 * seeded outcomes can't cross-gate — the logic is what's mirrored).
 *
 * Loads the compact dataset (data/gen3_{species,moves,typechart}.tsv). When this
 * moves into the Android app it loads the same data from assets and drives a
 * block Scene. main() runs a demo + a self-test.
 */
import java.io.File
...
class Dex(dir: String = "data") {
    ...
    init {
        ...
        File("$dir/gen3_moves.tsv").forEachLine {
            if (it.isBlank()) return@forEachLine
            val f = it.split("\t")
            val sc = if (f[13].isEmpty()) emptyList() else f[13].split(";").map {
                val (s, c) = it.split(":"); (STAT_MAP[s] ?: s) to c.toInt()
            }
...
fun main() {
    ...
    val teamA = listOf(
        mon("charizard", "flamethrower", "earthquake", "dragon-claw", "slash"),
        mon("blastoise", "surf", "ice-beam", "earthquake", "body-slam"),
        mon("venusaur", "razor-leaf", "sludge-bomb", "earthquake", "body-slam"))
    val teamB = listOf(
        mon("gengar", "shadow-ball", "sludge-bomb", "thunderbolt", "psychic"),
        mon("gyarados", "surf", "earthquake", "ice-beam", "body-slam", ab = "intimidate"),
        mon("machamp", "cross-chop", "earthquake", "rock-slide", "body-slam"))

    println("=== DEMO: 3v3 (Kotlin engine) ===")
    Battle(dex, teamA, teamB, seed = 7).run()

    // self-test: seeded battles must resolve with a live winner, no hangs
    print("\n=== self-test: 200 seeded battles resolve … ")
    var ok = 0
    for (s in 0 until 200) {
        val a = teamA.map { Mon(dex, it.species.name, moves = it.moves) }
        val b = teamB.map { Mon(dex, it.species.name, moves = it.moves) }
        val w = Battle(dex, a, b, seed = s.toLong()) { }.run()
        if (w != null && !w.fainted) ok++
    }
    println("$ok/200 ${if (ok == 200) "✅" else "❌"}")
    if (ok != 200) kotlin.system.exitProcess(1)
}
